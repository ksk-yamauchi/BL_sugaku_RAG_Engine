...
3. 🌟【タスク命名における対象テキストの尊重】: タスク（技能）を抽出・命名する際、テキスト上で操作の対象となっている数式表現（例：「単項式」「多項式」「整式」の違い）や、指示のニュアンス（例：「指定された文字」「特定の文字」）は、過去の登録リストに過剰に引きずられることなく、今回のテキストの記述を忠実に反映させてタスク名に含めてください。
...
def main():
    print("=== 🏁 【Ver 14.5 抽出対象尊重ガードレール ＆ 大問分離版】Phase 1 起動 ===")
