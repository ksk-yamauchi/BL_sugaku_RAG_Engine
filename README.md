...
* **プロンプト制御とJSON自動修復機構**: AIの出力ブレやLaTeX特有のエスケープ漏れによるクラッシュを防ぐため、プロンプトでの厳格なフォーマット指定に加え、Python側でバックスラッシュや改行を自動検知・修復するカスタムパーサーを搭載。ノイズのない美しいデータベース構築と長時間の安定稼働を両立しています。
...
* **スマート・ルーティング（動的ルート構築）**: ユーザーの検索意図（インプット優先か、解法ステップ優先か）を検知し、大問と確認問題が分離されたオントロジー構造を活かして、最適な学習ルート（第一アクション〜第三アクション）を動的に構築・展開します。
* **対話型 / マルチモーダル AIチューター**: 解けない問題や手書きノートの画像から、直ちに答えを教えるのではなく「ティーチングの意図（解説要約）」と「前提知識（ゼロアクション）」を提示して自律的解決を支援します。
* **主従動画の視覚的バッジ表示とXAI**: 動画サジェスト時に「💡 【メイン解説】」「⏪ 【前提・復習】」等のバッジと、先生の「解説の狙い」を明示し、生徒が今の自分に合ったコンテキストの動画を迷わず選択できます。見たいチャプターからのピンポイント再生にも対応しています。
...
│   ├── 📜 build_vector_db.py               # グローバルベクトルDB構築スクリプト (Ver 14.5 対応)
│   ├── 📜 export_global_obsidian_vault_mext.py # Obsidian用ZIPパッケージ出力 (Ver 14.5 対応)
...
│       ├── 📜 phase1_text_analysis_ontology.py # Phase 1: デュアルエンジン抽出 (Ver 14.5 大問/確認問題分離版)
│       ├── 📜 phase2_video_analysis.py     # Phase 2: 動画解析 (中問小問アセンブリ・LaTeXノイズ防止版) 
│       └── 📜 phase3_alignment_graph.py    # Phase 3: 動的補完 & 粒度吸収アライメント (Ver 14.5)
│
└── 📱 BL_sugaku_I_Frontend/                 # フロントエンド (Streamlit Web UI)
    ├── 📄 .env                             # フロントエンド用 APIキー設定
    ├── 📜 app.py                           # 生徒向け Streamlit AIチューター アプリ (UI Ver 4.5)
