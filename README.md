...
* **スマート・ルーティング（動的ルート構築）**: ユーザーの検索意図（インプットか、解法ステップか）を検知し、最適な学習ルート（第一アクション〜第三アクション）を動的に構築・展開します。また、タスク配列と知識配列を活用した多段引き当てにより、検索キーワードに関わらず確実に関連問題へナビゲートします。
...
* **問題中心のGNN-KTスキルツリー (Graph RAG)**: 問題ルートに入った際、本命の問題自身を「📍現在地」として中心に据え、そこから「⬛ 測られるタスク」と「🟦 必要な知識」へ放射状にエッジが伸びるインタラクティブなツリーを展開します。
...
└── 📱 BL_sugaku_I_Frontend/                 # フロントエンド (Streamlit Web UI)
    ...
    ├── 📜 app.py                           # 生徒向け Streamlit AIチューター アプリ (UI Ver 4.6)
