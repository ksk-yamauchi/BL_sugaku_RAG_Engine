...
* **スマート・ルーティング（動的ルート構築）**: ユーザーの検索意図（インプットか、解法ステップか）を検知し、最適な学習ルート（第一アクション〜第三アクション）を動的に構築・展開します。
* **概念ルートの純化（ノイズ排除アルゴリズム）**: インプットルートでの問題提示時、別のタスク（応用要素）が混ざった問題を徹底的に排除。純粋にそのタスク「だけ」を測る基本問題のみを提示し、初学者の躓きを防ぎます。
* **タスク駆動の類題サジェスト**: 問題検索時の「横展開」において、単なる類似テキストではなく「同じタスク（技能）を測る問題」を真の類題として提示します。
...
* **問題中心のGNN-KTスキルツリー (Graph RAG)**: 問題ルートに入った際、本命の問題自身を「📍現在地」として中心に据え、そこから「⬛ 測られるタスク」と「🟦 必要な知識」へ放射状にエッジが伸びるインタラクティブなツリーを展開します。クリティカルパスの強調（エッジの色分け）にも対応。
* **ラジオボタン連動型スキャッフォールディングUI**: 複数の本命問題が提示された際、ラジオボタンの切り替えに連動して「最適な大問（第一アクション）」や「前提確認（次点）」、「知識ツリー（Graph RAG）」などすべてのUIが動的に再描画・追従し、学習の迷いを防ぐ最強の導線を提供します。
...
└── 📱 BL_sugaku_I_Frontend/                 # フロントエンド (Streamlit Web UI)
    ...
    ├── 📜 app.py                           # 生徒向け Streamlit AIチューター アプリ (UI Ver 4.17.2)
