...
[ Phase 0 ] phase0_pdf_to_md.py
   └─ PDFテキスト抽出・LaTeX数式変換・図形/グラフの言語化 (動的再アップロード対応)
           │
           ▼
[ Phase 1 ] phase1_text_analysis_ontology.py
...
[ Phase 3 ] phase3_alignment_graph.py
 ...
 ─────────────────────────────────────────────────────────────
 [ バックエンド統合処理 ]
   ├─ build_index_master.py ➔ lecture_index_master.json の生成
   ├─ build_vector_db.py ➔ 動画カタログ統合・【大問と確認問題の独立ベクトル化＆配列保持(Array Ready)】
   ├─ export_global_obsidian_vault_mext.py ➔ 【大問/確認問題の視覚的分割出力＆双方向リンク対応】
...
* **グローバルDB構築 (`build_vector_db.py`)**:
  ...
  * **[配列構造の保持 (Array Ready)]** 従来行っていた「1つの問題につき代表概念を1つに絞って上書きする不可逆圧縮」を廃止。問題データが持つ「タスクの配列（`linked_task_names`）」と「知識の配列（`linked_knowledge_names`）」をそのまま独立したメタデータとしてDBへ格納します。
...
* **Obsidian出力 (`export_global_obsidian_vault_mext.py`)**:
  ...
  * **[GNN-KT推論ベースの双方向リンク出力]** DB構築で格納された「タスク配列」と「知識配列」を読み出し、問題ノードのMarkdown内に **「⬛ 測られるタスク (技能)」「🟦 必要な知識 (前提条件)」** として双方向リンク（`[[ ]]`）で展開します。
