...
[ Phase 0 ] phase0_pdf_to_md.py
   └─ PDFテキスト抽出・LaTeX数式変換・図形/グラフの言語化 (動的再アップロード対応)
           │
           ▼
[ Phase 1 ] phase1_text_analysis_ontology.py (Ver 13.1.x)
   └─ デュアルエンジン（タスク/知識）抽出・マスター照合・自動採番・【記憶の継承】による表記揺れ防止
           │
           ▼
[ Phase 2 ] phase2_video_analysis.py (Ver 2.5.x)
   └─ 役割判定・極細チャプター分割・板書OCR生成・中問/小問対応アセンブリ (プロンプト制御版)
           │
           ▼
[ Phase 3 ] phase3_alignment_graph.py (Ver 13.2.x)
 ...
 ─────────────────────────────────────────────────────────────
 [ バックエンド統合処理 ]
   ├─ build_vector_db.py ➔ ベクトルDB構築・【オートワイヤリング（実体結合）】(Ver 13.x 対応・APIキー自動ローテーション搭載)
   ├─ export_global_obsidian_vault_mext.py ➔ Obsidian Vault ZIP 出力・【スマートリンク化】
...
### 1. 環境変数の設定 (`.env`) と複数APIキーのローテーション仕様
```bash
...
バックエンドディレクトリに `.env` ファイルを作成し、Gemini APIキーを設定します。カンマ区切りで複数のキーを登録することで、制限到達時（429エラー時）に自動で次のキーへ切り替わります。
```env
